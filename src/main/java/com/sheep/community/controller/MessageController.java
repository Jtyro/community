...
public class MessageController implements CommunityConstant {
    ...
    public String listNotice(Model model) {
        ...
        if (message != null) {
            Map<String, Object> messageVO = putMessage(user, message, TOPIC_COMMENT);
            model.addAttribute("commentNotice", messageVO);
        }

        message = messageService.findLatestNotice(user.getId(), TOPIC_LIKE);
        if (message != null) {
            Map<String, Object> messageVO = putMessage(user, message, TOPIC_LIKE);
            model.addAttribute("likeNotice", messageVO);
        }

        message = messageService.findLatestNotice(user.getId(), TOPIC_FOLLOW);
        if (message != null) {
            Map<String, Object> messageVO = putMessage(user, message, TOPIC_FOLLOW);
    ...
    private Map<String, Object> putMessage(User user, Message message, String topic){
        Map<String, Object> messageVO = new HashMap<>();
        messageVO.put("message", message);

        putContentDataToVO(message, messageVO);

        int noticeCount = messageService.findNoticeCount(user.getId(), topic);
        messageVO.put("count", noticeCount);
        int unread = messageService.findUnreadNoticeCount(user.getId(), topic);
        messageVO.put("unread", unread);
        return messageVO;
    }
    private void putContentDataToVO(Message message, Map<String, Object> messageVO){
    ...
    public String getNoticeDetail(@PathVariable("topic") String topic, Model model, Page page) {
        ...
        if (messageList != null) {
            for (Message message : messageList) {
                ...
                putContentDataToVO(message, map);
